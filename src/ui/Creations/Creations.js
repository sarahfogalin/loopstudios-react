    ...
    return (
        <div>
            ...
            {creationsData.map((creation, index) => {
                const imgSrc = dimensions.width <= MAX_MOBILE_SCREEN ? creation.mobileSrc : creation.desktopSrc;
                return (
                    <div className='creation-container' key={`${creation.label}${index}`}>
                        <div className='creation-label'>
                            <h3>{creation.lineOne}</h3>
                            <h3>{creation.lineTwo}</h3>
                        </div>
                        <span className='overlay'>
                            <img src={imgSrc} alt={creation.label} className='creation-image center' />
                        </span>
                    </div>
                )
            })}
